...
async function handleCasinoBet(
  ...
  if (cmd === "writeBet") {
    // ========================================================================
    // CASINO BET LOGIC: Use roundId to consolidate multiple requests
    // ========================================================================
    // Multiple requests may come for the same round:
    //   1. Initial bet: {bet: 100, win: 0, roundId: "r_123"}
    //   2. Settlement: {bet: 100, win: 150, roundId: "r_123"}
    // These should UPDATE the same record, not create duplicates!
    // ========================================================================

    // Check if record exists for this roundId + user
    ...
    // If no roundId but we have transactionId, check by wagerCode (fallback)
    ...
    // Calculate new balance based on whether this is CREATE or UPDATE
    let newBalance: Decimal;
    if (existingBet) {
      // UPDATE case: undo old profitNLoss, apply new one
      console.log(
        `[CASINO] Updating existing bet. roundId: ${roundId}, old profitNLoss: ${existingBet.profitNLoss}, new profitNLoss: ${profitNLoss}`,
      );
      const oldProfit = existingBet.profitNLoss || new Decimal(0);
      newBalance = userBalance.sub(oldProfit).add(profitNLoss);
    } else {
      // CREATE case: just add the new profitNLoss
      console.log(
        `[CASINO] Creating new bet. roundId: ${roundId}, profitNLoss: ${profitNLoss}`,
      );
      newBalance = userBalance.add(profitNLoss);
    }

    // Check Insufficient Balance (only for initial bet)
    if (!existingBet && bet.gt(userBalance)) {
      console.error(
        `[BALANCE ERROR] Insufficient balance. Bet: ${bet.toString()}, Available: ${userBalance.toString()}`,
      );
      return createAndLogResponse(
    ...
    await db.$transaction(async (tx) => {
      ...
      if (existingBet) {
        ...
        console.log(`[CASINO] Updating BettingRecord id=${existingBet.id}`);
        await tx.bettingRecord.update({
          ...
          data: {
            ...
            status: status,
            // Keep category from first request, or update if needed
            wagerCode: transactionId, // Track the latest transactionId
          },
        });
      } else {
        // CREATE new record
        console.log(
          `[CASINO] Creating new BettingRecord for roundId=${roundId}`,
        );
  ...
  if (cmd === "rollback") {
    // Find the bet by roundId first (primary), fallback to wagerCode
    let existingTx = null;
    if (roundId) {
