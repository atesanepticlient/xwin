...
function verifySignature(
  ...
  if (!signatureHeader || !USER_SECRET) {
    console.warn(
      "[AUTH] Verification failed: Missing x-signature header or GREGMORN_SECRET env variable.",
    );
    return false;
  }
...
function parseRoundId(payload: Record<string, any>): string | null {
  // Check direct payload top-level attributes first
  if (payload.roundId || payload.round_id) {
    return String(payload.roundId || payload.round_id);
  }

  const info = payload.info;
  if (!info) return null;
  if (typeof info === "object") {
    return info.roundId || info.round_id || null;
  }
...
/**
 * Sports betting identifier rule
 */
...
async function handleSportsBet(
  ...
  if (cmd === "writeBet") {
    ...
    if (existingTx) {
      console.warn(
        `[IDEMPOTENCY] Duplicate sports transaction: '${transactionId}'. Returning balance.`,
      );
      return createAndLogResponse({
        status: "success",
        error: "",
    ...
    if (bet.gt(userBalance)) {
      console.error(
        `[BALANCE ERROR] Insufficient balance. Bet: ${bet.toString()}, Balance: ${userBalance.toString()}`,
      );
      return createAndLogResponse(
        {
          status: "fail",
...
// ============================================================================
// CASINO BETTING HANDLER (CONSOLIDATED ROUND LOGIC)
// ============================================================================
...
async function handleCasinoBet(
  ...
  const { cmd, login, transactionId, gameId, round_finished } = payload;
  ...
  if (cmd === "writeBet") {
    // ------------------------------------------------------------------------
    // 1. LOOK UP EXISTING BET RECORD
    // Priority 1: Match by user + roundId (linking initial bet with result)
    // Priority 2: Match by wagerCode / transactionId
    // ------------------------------------------------------------------------
    let existingBet = null;

    if (roundId) {
      existingBet = await db.bettingRecord.findFirst({
        where: {
          userId: user.id,
          roundId: roundId,
        },
    ...
    if (!existingBet && transactionId) {
      existingBet = await db.bettingRecord.findFirst({
        where: {
          userId: user.id,
          wagerCode: transactionId,
        },
      });
    }
    ...
    // ------------------------------------------------------------------------
    // 2. IDEMPOTENCY CHECK
    // If exact transactionId arrives again AND is already settled
    // ------------------------------------------------------------------------
    if (
      existingBet &&
      existingBet.wagerCode === transactionId &&
      existingBet.status === "SETTLED"
    ) {
      console.warn(
        `[IDEMPOTENCY] Exact transactionId '${transactionId}' already processed. Returning current balance.`,
      );
      return createAndLogResponse({
        status: "success",
        error: "",
    ...
    if (existingBet) {
      // UPDATE: Revert previously stored profitNLoss, then apply the new net PnL
      const oldProfit = existingBet.profitNLoss
        ? new Decimal(existingBet.profitNLoss)
        : new Decimal(0);

      newBalance = userBalance.sub(oldProfit).add(profitNLoss);
      console.log(
        `[CASINO UPDATE] Round '${roundId}'. Old PnL: ${oldProfit.toString()}, New PnL: ${profitNLoss.toString()}`,
      );
    } else {
      // CREATE: Insufficient balance check (only on initial wager placement)
      if (bet.gt(userBalance)) {
        console.error(
          `[BALANCE ERROR] Insufficient balance. Bet: ${bet.toString()}, Balance: ${userBalance.toString()}`,
        );
      ...
      newBalance = userBalance.add(profitNLoss);
      console.log(
        `[CASINO CREATE] New Round '${roundId}', Tx: '${transactionId}', Net PnL: ${profitNLoss.toString()}`,
      );
    }
    ...
    await db.$transaction(async (tx) => {
      ...
      if (existingBet) {
        // UPDATE existing record for this roundId
        await tx.bettingRecord.update({
          where: { id: existingBet.id },
          data: {
            betAmount: bet.gt(0) ? bet : existingBet.betAmount,
            profitNLoss: profitNLoss,
            status: status,
            wagerCode: transactionId || existingBet.wagerCode,
          },
        });
      } else {
  ...
  if (cmd === "rollback") {
    ...
    if (roundId) {
      existingTx = await db.bettingRecord.findFirst({
    ...
    if (!existingTx && transactionId) {
      existingTx = await db.bettingRecord.findFirst({
        where: { userId: user.id, wagerCode: transactionId },
      });
    }

    if (!existingTx || existingTx.status === "CANCELED") {
      console.warn(
        `[CMD: rollback] Casino transaction '${transactionId}' / roundId '${roundId}' ${!existingTx ? "not found" : "already CANCELED"}. Skipping.`,
      );
      return createAndLogResponse({
        status: "success",
        error: "",
...
async function handleWebhook(
  ...
  try {
    ...
    if (!["getBalance", "writeBet", "rollback"].includes(cmd)) {
      console.error(`[COMMAND ERROR] Unknown command: '${cmd}'`);
      return createAndLogResponse(
        {
          status: "fail",
          error: "cmd_not_found",
    ...
    if (cmd === "getBalance") {
      console.log(`[CMD: getBalance] Balance request for ${user.playerId}`);
      return createAndLogResponse({
        status: "success",
        error: "",
        login: user.playerId,
    ...
    // Route based on game identifier
    if (isSportsBetting(gameId)) {
      console.log(`[ROUTING] Sports betting detected (${gameId})`);
      return await handleSportsBet(req, user, payload, currency);
    } else {
      console.log(`[ROUTING] Casino betting detected (${gameId})`);
