...
import {
  BettingOption,
  LiveMatchCardProps,
} from "./category/live-match-selection";
import { EsportsMarket, EsportsMatchCardProps } from "./esports-match-card";
...
export const Live1xDataWarapper = ({
  key,
  children,
}: {
  key: "LIVE_SPORTS" | "SPORTS" | "TOURNAMENT";
  ...
  const [totalTournaments, setTotalTournament] = useState(
    ...
    eventSource.onmessage = (event) => {
      try {
        ...
        if (data.success) {
          ...
          setTotalTournament(data.data.tournaments.length);
          setTotalEsports(
  ...
  return <div>Live1xDataWarapper</div>;
};
